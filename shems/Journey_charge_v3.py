...
def main():
    ...
    mpg_temp_shift(inputs, response)
    results_journey(inputs, response, opt_response)
    charge_time = time_charge(inputs, response, opt_response)
    print(charge_time)
    distance_km = API.journey_distance(response)
    print(distance_km)
    traffic_time = API.journey_time_traffic(response)
    print(traffic_time)
    carbon_kg = journey_carbon_cost(inputs, response, opt_response)
    print(carbon_kg)
...
def plug_out(inputs, response):
    ...
    plug_out_time = destination_arrival_time - API.journey_time_traffic(response)
    return plug_out_time


def time_charge(inputs, response, opt_response):
    ...
    final_charge_kwh = final_kwh(inputs, response, opt_response)
...
def heating(inputs, response):
    ...
    plug_out_time = plug_out(inputs, response)
...
def shift_charge(inputs, response, opt_response):
    """Calculates a shifted charge requirement based on the calculated effects of individual journey characteristics"""
    heat_effect = heating(inputs, response)
    cool_effect = cooling(inputs, response)
    rain_effect = rain(inputs, response)
    temp_effect = temp(inputs, response)
    traffic_effect = range_traffic_shift(response, opt_response)
    ...
    init_charge_req = init_charge(inputs, response)
...
def final_charge(inputs, response, opt_response):
...
def results_journey(inputs, response, opt_response):
    ...
    print("Therefore", charge_time, "hours are needed to charge to requirement using a", inputs['Charge Rate'],
          "kW charger")
    pounds_saved = journey_savings(inputs, response, opt_response)
...
def avg_journey_vel(response, opt_response):
    ...
    intersections"""
    journey_dist_km = (API.journey_distance(response) + 0.000001)  # BODGE to avoid /0 error
    journey_time_opt_hours = round((API.journey_time_optimist(opt_response) / 60), 2)
...
def mpg_traffic_shift(response, opt_response):
...
def mpg_temp_shift(inputs, response):
    ...
    short_trips = trip_length_band(response)
...
def mpg_ice(inputs, response, opt_response):
    ...
    temp_effect_mpg = mpg_temp_shift(inputs, response)
    traffic_effect_mpg = mpg_traffic_shift(response, opt_response)
...
def petrol_cost(inputs, response, opt_response):
    ...
    mpg = mpg_ice(inputs, response, opt_response)
    ...
    journey_petrol_price = API.journey_distance(response) * petrol_p_per_km
    return journey_petrol_price


def journey_carbon_cost(inputs, response, opt_response):
    ...
    journey_co2_price = round(API.journey_distance(response) * petrol_co2_per_km, 2)
    return journey_co2_price


def journey_cost(inputs, response, opt_response):
    ...
    if vehicle_select == 1 or vehicle_select == 2:
        ...
        petrol_p_per_km = petrol_consump_rate * petrol_costs
        journey_price = API.journey_distance(response) * petrol_p_per_km

    return journey_price


def journey_savings(inputs, response, opt_response):
    ...
    trip_cost = journey_cost(inputs, response, opt_response)
    print('In an EV, this trip costs £', (round((trip_cost / 100), 2)))
    journey_petrol_price = petrol_cost(inputs, response, opt_response)
