...
def main():
    ...
    response = API.initialise_api_data(inputs, False)
    opt_response = API.initialise_api_data_optimistic(inputs, False)
    mpg_temp_shift(inputs, False, response)
    results_journey(inputs, False, response, opt_response)
    charge_time = time_charge(inputs, False, response, opt_response)
    print(charge_time)
    distance_km = API.journey_distance(inputs, False, response)
    print(distance_km)
    traffic_time = API.journey_time_traffic(inputs, False, response)
    print(traffic_time)
    carbon_kg = journey_carbon_cost(inputs, False, response, opt_response) + journey_cost(inputs, True, response, opt_response)
    print(carbon_kg)
...
def plug_out(inputs, reserve_journey, response):
    ...
    plug_out_time = destination_arrival_time - API.journey_time_traffic(inputs, reserve_journey, response)
    return plug_out_time


def time_charge(inputs, reserve_journey, response, opt_response):
    ...
    final_charge_kwh = final_kwh(inputs, reserve_journey, response, opt_response)
...
def heating(inputs, reserve_journey, response):
    ...
    plug_out_time = plug_out(inputs, reserve_journey, response)
...
def cooling(inputs, reserve_journey, response):
...
def rain(inputs, reserve_journey, response):
    ...
    rain_effect = precipdata[
                  plug_out_time.replace(year=2019):plug_out_time.replace(year=2019) + pd.Timedelta(
                      '1 day')].copy().iloc[0, 1]
    return rain_effect


def temp(inputs, reserve_journey, response):
...
def range_traffic_shift(inputs, reserve_journey, response, opt_response):
    """Assigns a shifted range value to an EV completing the journey specified based on traffic conditions."""
    journey_vel = avg_journey_vel(inputs, reserve_journey, response, opt_response)
...
def init_charge(inputs, reserve_journey, response):
    """Calculates the initial charge required to complete the specified journey input"""
    trip_distance = API.journey_distance(inputs, reserve_journey, response)
...
def shift_charge(inputs, reserve_journey, response, opt_response):
    """Calculates a shifted charge requirement based on the calculated effects of individual journey characteristics"""
    heat_effect = heating(inputs, reserve_journey, response)
    cool_effect = cooling(inputs, reserve_journey, response)
    rain_effect = rain(inputs, reserve_journey, response)
    temp_effect = temp(inputs, reserve_journey, response)
    traffic_effect = range_traffic_shift(inputs, reserve_journey, response, opt_response)
    ...
    init_charge_req = init_charge(inputs, reserve_journey, response)
...
def final_charge(inputs, reserve_journey, response, opt_response):
    ...
    shift_charge_req = shift_charge(inputs, reserve_journey, response, opt_response)
    add_charge = charge_add(inputs)
    charge_final = shift_charge_req + add_charge
    return charge_final


def final_kwh(inputs, reserve_journey, response, opt_response):
    """Converts the final charge requirement calculated in final_charge from a percentage into a kWh requirement"""
    final_charge_percent = final_charge(inputs, reserve_journey, response, opt_response)
    final_charge_kwh = (final_charge_percent / 100) * inputs['Battery Capacity']
    return final_charge_kwh


def results_journey(inputs, reserve_journey, response, opt_response):
    """Presents and prints the final results in the terminal"""
    final_charge_percent = round(final_charge(inputs, reserve_journey, response, opt_response), 2)
    ...
    print("Therefore", charge_time, "hours are needed to charge to requirement using a", inputs['Charge Rate'],
          "kW charger")
    pounds_saved = journey_savings(inputs, reserve_journey, response, opt_response)
...
def avg_journey_vel(inputs, reserve_journey, response, opt_response):
    ...
    intersections"""
    journey_dist_km = (API.journey_distance(inputs, reserve_journey, response) + 0.000001)  # BODGE to avoid /0 error
    journey_time_opt_hours = round((API.journey_time_optimist(inputs, reserve_journey, opt_response) / 60), 2)
...
def trip_length_band(inputs, reserve_journey, response):
    ...
    distance_km = API.journey_distance(inputs, reserve_journey, response)
...
def mpg_traffic_shift(inputs, reserve_journey, response, opt_response):
...
def mpg_temp_shift(inputs, reserve_journey, response):
    ...
    short_trips = trip_length_band(inputs, reserve_journey, response)
...
def mpg_ice(inputs, reserve_journey, response, opt_response):
    ...
    temp_effect_mpg = mpg_temp_shift(inputs, reserve_journey, response)
    traffic_effect_mpg = mpg_traffic_shift(inputs, reserve_journey, response, opt_response)
...
def petrol_cost(inputs, reserve_journey, response, opt_response):
    ...
    mpg = mpg_ice(inputs, reserve_journey, response, opt_response)
    ...
    journey_petrol_price = API.journey_distance(inputs, reserve_journey, response) * petrol_p_per_km
    return journey_petrol_price


def journey_carbon_cost(inputs, reserve_journey, response, opt_response):
    ...
    journey_co2_price = round(API.journey_distance(inputs, reserve_journey, response) * petrol_co2_per_km, 2)
    return journey_co2_price


def journey_cost(inputs, reserve_journey, response, opt_response):
    ...
    if vehicle_select == 1 or vehicle_select == 2:
        ...
        petrol_p_per_km = petrol_consump_rate * petrol_costs
        journey_price = API.journey_distance(inputs, reserve_journey, response) * petrol_p_per_km

    return journey_price


def journey_savings(inputs, reserve_journey, response, opt_response):
    ...
    trip_cost = journey_cost(inputs, reserve_journey, response, opt_response)
    print('In an EV, this trip costs £', (round((trip_cost / 100), 2)))
    journey_petrol_price = petrol_cost(inputs, reserve_journey, response, opt_response)
    print('In an ICE, this trip costs £', (round((journey_petrol_price / 100), 2)))
    trip_savings = round(((journey_petrol_price - trip_cost) / 100), 2)
    return trip_savings


def somerandomname(inputs, reserve_journey):
    response = API.initialise_api_data(inputs, reserve_journey)
    opt_response = API.initialise_api_data_optimistic(inputs, reserve_journey)
